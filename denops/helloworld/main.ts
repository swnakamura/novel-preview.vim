...
let index_url = new URL("./index.html", import.meta.url);
let bodyHTML = Deno.readTextFileSync(fromFileUrl(index_url));
app.get("/", async (req) => {
  await req.respond({
    status: 200,
    headers: new Headers({
...
export async function main(denops: Denops): Promise<void> {
  denops.dispatcher = {
    ...
    async updateServer(): Promise<unknown> {
      let bufferContentList = (await denops.eval("getline(1, '$')")) as [
        string,
      ];
      ...
      bodyHTML = create_index_html(bufferContent);
